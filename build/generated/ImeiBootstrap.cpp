...
#define TAG "ro.mihaiblaga.imei"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Imei_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Imei.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::ImeiBindings::lookupGeneratedInit(
...
static void Imei_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Imei_getBinding)->GetFunction());
}

static void Imei_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::ImeiBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry ImeiBinding = {
	"ro.mihaiblaga.imei",
	Imei_init,
	Imei_dispose
};
...
Java_ro_mihaiblaga_imei_ImeiBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ro.mihaiblaga.imei", &ImeiBinding);
	titanium::KrollBindings::addExternalLookup(&(::ImeiBindings::lookupGeneratedInit));
}
