...
Handle<Value> ExampleProxy::getImei(const Arguments& args)
{
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(ExampleProxy::javaClass, "getImei", "()Ljava/lang/String;");
		if (!methodID) {
...
Handle<Value> ExampleProxy::getter_imei(Local<String> property, const AccessorInfo& info)
{
	LOGD(TAG, "get imei");
	HandleScope scope;
